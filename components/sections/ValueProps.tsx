        ...
        <div className={`grid grid-cols-1 gap-12 sm:grid-cols-2 ${items.length === 4 ? 'lg:grid-cols-4' : items.length === 5 ? 'lg:grid-cols-5' : 'lg:grid-cols-3'}`}>
          {items.map((item, index) => (
            <div key={index} className="flex flex-col items-center text-center">
              <div className="h-[70px] w-[70px] rounded-full border border-[#c5a059] bg-[#c5a059]/10 flex items-center justify-center text-[#c5a059] mb-8 shadow-sm transition-colors duration-300">
                {item.icon ? React.cloneElement(item.icon as React.ReactElement, { className: 'h-[29px] w-[29px]' }) : (
